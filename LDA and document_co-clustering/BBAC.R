...
for (i in 1:length(k_range)){
  ...
  # Running time
  start <- proc.time()
  bbac_result_raw <- bbac(b, k = num_cluster, l = num_cluster, distance = "d", scheme = 5)
  time = proc.time() - start
  Running_Time[i] =as.numeric(time[3])
  
  # Store results
  bbac_results = as.data.frame(bbac_result_raw$R) %>% mutate(n = 1:nrow(b))
  gather1 = tidyr::gather(bbac_results, "cluster", "indicator", -n)%>% filter(indicator == 1) 
  gather2 = gather1[order(gather1$n),] %>% mutate(new_cluster = substr(cluster,2,3)) %>% select(new_cluster)
  BBAC_results = gather2$new_cluster
  
  nn = data.frame(as.numeric(BBAC_results))
  BBAC = cbind(BBAC, nn)
}
...
#write.csv(BBAC, "BBAC_results_stemming.csv")
#write.csv(Running_Time, "BBAC_running_time_stemming.csv")
